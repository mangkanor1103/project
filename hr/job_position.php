    ...
    <div class="container mx-auto px-4 max-w-7xl">
        ...
        <div class="mb-6">
            <div class="flex border-b border-gray-200">
                <button onclick="showTab('add')" class="tab-button px-6 py-3 text-indigo-600 border-b-2 border-indigo-600 font-medium">Add Position</button>
                <button onclick="showTab('list')" class="tab-button px-6 py-3 text-gray-500 font-medium">Position List</button>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php foreach ($positions as $position): ?>
                                <tr id="pos-row-<?php echo $position['id']; ?>">
                                    <td class="px-6 py-4 whitespace-nowrap font-medium"><?php echo htmlspecialchars($position['position_name']); ?></td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <button onclick="editPosition(<?php echo $position['id']; ?>, '<?php echo addslashes($position['position_name']); ?>', '<?php echo addslashes($position['position_description'] ?? ''); ?>')"
                                            class="text-indigo-600 hover:text-indigo-900 mr-3">
                                            Edit
                                        </button>
                                        <button onclick="confirmDelete(<?php echo $position['id']; ?>, '<?php echo addslashes($position['position_name']); ?>', <?php echo $position['employee_count']; ?>)"
                                            class="text-red-600 hover:text-red-900" 
                                            <?php echo ($position['employee_count'] > 0) ? 'title="This will require reassigning employees"' : ''; ?>>
                                            Delete
                                        </button>
                                        <button onclick="viewEmployeesByPosition('<?php echo addslashes($position['position_name']); ?>')"
        ...
        <div id="assign-tab" class="tab-content hidden bg-white shadow-lg rounded-lg p-6">
            ...
            <?php else: ?>
                <form method="POST" action="">
                    <div class="mb-6">
                        <label for="assign_position_id" class="block text-sm font-medium text-gray-700 mb-1">Select Position</label>
                        <select id="assign_position_id" name="position_id" required
                            ...
                            <?php foreach ($positions as $position): ?>
                                <option value="<?php echo $position['id']; ?>">
                                    <?php echo htmlspecialchars($position['position_name']); ?> (<?php echo $position['employee_count']; ?> employees)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Current Position</th>
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <form method="POST" action="" id="editForm">
                ...
                <div class="mb-4">
                    <label for="edit_position_name" class="block text-sm font-medium text-gray-700 mb-1">Position Name</label>
                    <input type="text" id="edit_position_name" name="position_name" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div class="mb-4">
                    <label for="edit_position_description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <form method="POST" action="" id="deleteForm">
                ...
                <div id="reassignSection" class="mb-4 hidden">
                    <label for="reassign_to_id" class="block text-sm font-medium text-gray-700 mb-1">Reassign employees to:</label>
                    ...
                    <p class="text-sm text-gray-500 mt-1">All employees in the deleted position will be moved to this position.</p>
                </div>
